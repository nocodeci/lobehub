...
const AskAgentCommands = memo(() => {
  ...
  // Check if Branding Name matches the query
  const showBrandingAI = !mentionQuery || BRANDING_NAME.toLowerCase().includes(mentionQuery);
  ...
  return (
    <Command.Group heading={t('cmdk.mentionAgent')}>
      ...
      {showBrandingAI && (
        <Command.Item
          onSelect={() => handleAgentSelect(inboxAgentId, BRANDING_NAME, DEFAULT_INBOX_AVATAR)}
          value={`@${kebabCase(BRANDING_NAME)}`}
        >
          <Avatar avatar={DEFAULT_INBOX_AVATAR} background={'transparent'} shape="square" size={18} />
          <div className={styles.itemContent}>
            <div className={styles.itemLabel}>@{BRANDING_NAME}</div>
          </div>
        </Command.Item>
      )}
