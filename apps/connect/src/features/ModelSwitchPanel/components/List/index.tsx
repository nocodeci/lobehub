...
export const List: FC<ListProps> = ({
  ...
  const [aiProviderList, enabledAiProviders] = useAiInfraStore((s) => [
    s.aiProviderList,
    s.enabledAiProviders,
  ]);
  const serverAiProviders = useServerConfigStore((s) => s.serverConfig.aiProvider);
      ...
      return (
        <ListItemRenderer
          ...
          isProviderEnabled={(providerId) => {
            const enabledByServerConfig = serverAiProviders?.[providerId]?.enabled;
            const enabledByList = aiProviderList?.find((p) => p.id === providerId)?.enabled;
            const enabledByServer = enabledAiProviders?.some((p) => p.id === providerId);

            if (enabledByServerConfig === false) return false;
            if (enabledByList === false) return false;
            if (enabledByServer === false) return false;
