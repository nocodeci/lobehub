...
export const useSkillConnect = ({ identifier, serverName, type }: UseSkillConnectOptions) => {
  ...
  const handleLobehubConnect = useCallback(async () => {
    console.log('[SkillConnect] handleLobehubConnect called for:', identifier);

    if (lobehubServer?.isConnected) {
      console.log('[SkillConnect] Already connected, skipping');
      return;
    }
    ...
    try {
      ...
      if (!provider) {
        console.error('[SkillConnect] Provider not found:', identifier);
        return;
      }

      console.log('[SkillConnect] Getting authorize URL for provider:', provider.id);
      ...
      console.log('[SkillConnect] Got authorize URL:', authorizeUrl);

      openOAuthWindow(authorizeUrl, identifier);
    } catch (error) {
      console.error('[SkillConnect] Failed to get authorize URL:', error);
      // Show error to user via notification or alert
      if (error instanceof Error && error.message.includes('Authentication required')) {
        alert('Vous devez d\'abord vous connecter à LobeHub Market pour utiliser cette intégration.');
      }
    } finally {
