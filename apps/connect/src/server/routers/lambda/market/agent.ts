...
// Authenticated-optional procedure for agent management
// Allows unauthenticated access but will attempt to fetch user info if available
const agentProcedure = publicProcedure
  .use(serverDatabase)
  .use(marketUserInfo)
  .use(marketSDK)
  .use(async ({ ctx, next }) => {
    // If no userId, skip fetching market token
...
export const agentRouter = router({
  ...
  checkOwnership: agentProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      // Check if we have Market authentication
      ...
      // Check if trustedClientToken can be generated (requires MARKET_TRUSTED_CLIENT_SECRET and MARKET_TRUSTED_CLIENT_ID)
      let hasTrustedClientAuth = false;
      if (userInfo && userInfo.email) {
        const trustedToken = generateTrustedClientToken(userInfo);
        hasTrustedClientAuth = !!trustedToken;
      }

      const hasMarketAuth = hasTrustedClientAuth || !!accessToken;

      // If no Market authentication, assume local mode - user owns their local agents
      if (!hasMarketAuth) {
        log('No Market authentication, assuming local ownership');
        return {
          exists: false,
          isOwner: true,
          originalAgent: null,
  ...
  /**
   * Unified publish/create flow for local activation
   * Sets the agent as "active" for local use (WhatsApp, etc.) without requiring Market API
   * POST /market/agent/publishOrCreate
   */
  publishOrCreate: agentProcedure.input(publishOrCreateSchema).mutation(async ({ input, ctx }) => {
    ...
    log('Auth check: hasTrustedClientAuth=%s, hasAccessToken=%s, hasMarketAuth=%s', hasTrustedClientAuth, !!accessToken, hasMarketAuth);

    // If no Market authentication, use local-only activation
    if (!hasMarketAuth) {
      log('No Market authentication, using local activation mode');

      // Generate a local identifier if not provided
      if (!finalIdentifier) {
        finalIdentifier = generateMarketIdentifier();
        isNewAgent = true;
      }

      // Return success - the agent is now "activated" locally
      // The frontend will update the agent's marketIdentifier which enables integrations
      return {
    ...
    // Full Market SDK flow for authenticated users
    try {
      ...
      // Step 4: Ensure the agent is published (active)
      log('Ensuring agent is published: %s', finalIdentifier);
      await ctx.marketSDK.agents.publish(finalIdentifier);
