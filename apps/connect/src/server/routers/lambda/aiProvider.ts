...
import { canUseBYOK } from '@/libs/subscription';
...
export const aiProviderRouter = router({
  ...
  updateAiProviderConfig: aiProviderProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      // Check if the update contains keyVaults (BYOK) — enforce plan restriction
      const hasKeyVaults = input.value.keyVaults && Object.keys(input.value.keyVaults).some(
        (k) => input.value.keyVaults![k] !== undefined && input.value.keyVaults![k] !== '',
      );
      if (hasKeyVaults) {
        ...
        if (!byokCheck.allowed) {
          throw new TRPCError({
            code: 'FORBIDDEN',
            message: byokCheck.message || 'BYOK est disponible à partir du plan Pro.',
          });
        }
      }
