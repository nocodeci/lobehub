...
import { canCreateAgent } from '@/libs/subscription';
...
export const agentRouter = router({
  ...
  createAgent: agentProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      // Enforce agent creation limit based on subscription plan
      const currentPlan = await getSubscriptionPlan(ctx.userId);
      const agentCount = await ctx.agentModel.countAgents();
      const check = canCreateAgent(currentPlan, agentCount);

      if (!check.allowed) {
        throw new TRPCError({
          code: 'FORBIDDEN',
          message: check.message || `Limite de ${check.limit} agents atteinte pour votre plan.`,
        });
      }
