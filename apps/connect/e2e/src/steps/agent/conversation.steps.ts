...
Given('用户进入 Connect 对话页面', async function (this: CustomWorld) {
  ...
  console.log('   📍 Step: 查找 Connect...');
  // Find and click on "Connect" agent in the sidebar/home
  const connectAgent = this.page.locator('text=Connect').first();
  await expect(connectAgent).toBeVisible({ timeout: WAIT_TIMEOUT });

  console.log('   📍 Step: 点击 Connect...');
...
Given('用户已发送消息 {string}', async function (this: CustomWorld, message: string) {
  ...
  // Assistant messages are left-aligned .message-wrapper elements that contain "Connect" title
  ...
  const assistantMessage = this.page.locator('.message-wrapper').filter({
    has: this.page.locator('text=Connect'),
  });
